...
function cityfunc() {
    ...
    if (city.trim().length == 0) {
        ...
        document.querySelector(":root").style.setProperty("--pseudo-bg", `#0B3586`);
        ...
        .then(data => {
            if (data.cod == 404) {
                ...
                checktime()
                document.getElementById("info-1").innerHTML = `TEMPRATURE : ${data.main.temp} K`;
                document.getElementById("info-2").innerHTML = `PRESSURE : ${data.main.pressure} hPa`;
                document.getElementById("info-3").innerHTML = `DESCRIPTION : ${data.weather[0].description}`;
                document.getElementById("info-4").innerHTML = `WIND-SPEED : ${data.wind.speed} m/s`;
                document.getElementById("info-5").innerHTML = `VISIBILITY : ${data.visibility} m`;
                document.getElementById("info-6").innerHTML = `HUMIDITY : ${data.main.humidity} %`;
                window.scrollTo(0, 550);
